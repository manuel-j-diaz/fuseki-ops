...
docker pull ghcr.io/manuel-j-diaz/fuseki-ops/fuseki:latest
...
Controlled by the `command` in `docker-compose.yaml`. Two options are provided — comment/uncomment to switch (rebuild is not required, restart is enough):
